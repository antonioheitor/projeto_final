...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fotomsg"]["tmp_name"], $target_file)) {
        echo "The file ". htmlspecialchars( basename( $_FILES["fotomsg"]["name"])). " has been uploaded.";
    } else {
...
if ( isset($_POST["mensagem"]) ) {
    ...
    $query = "INSERT INTO mensagens (mensagem_chat, imagem_chat, grupo_id_grupo, users_id_users, data_msg) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)";

    if (mysqli_stmt_prepare($stmt, $query)) {
        mysqli_stmt_bind_param($stmt, 'ssii', $mensagem, $imagem_chat ,$grupo_id_grupo, $id);
